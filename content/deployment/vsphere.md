...
vSphere is common for industrial and on-premises deployments with no public cloud reachable — plant networks, air-gapped-adjacent sites, existing VMware estates. `cluster.driver` is always `talos`; there's no managed Kubernetes offering on vSphere to target instead.
...
VMs deploy from the Talos OVA via guestinfo — vSphere's native mechanism for passing config into a guest at creation time, the same role cloud-init plays on AWS or Azure. Machine secrets and machine config are generated inline as part of the deploy, so there's no separate config-generation step the way Hyper-V needs one.
...
Set vCenter credentials as environment variables — Windsor exports `VSPHERE_SERVER`/`VSPHERE_USER`/`VSPHERE_ALLOW_UNVERIFIED_SSL` from the `vsphere.*` block below, and the password from a `${secret(...)}` reference, so they never land in `values.yaml` in plaintext:
...
`datacenter`, `cluster`, `datastore`, and `network` are all required — Windsor refuses to compose the blueprint without them, since there's no sensible default for infrastructure it doesn't own. Names must match vCenter's inventory exactly.

As on Hetzner and Hyper-V, there's no `cluster.pools` — `cluster.controlplanes.count` / `cluster.workers.count` set node counts directly, sized by `cluster.{controlplanes,workers}.cpu`/`memory`, and `topology` is inferred from the counts.
...
vSphere VMs get real routed IPs on your network, so an in-cluster load balancer works the way it would on bare metal. `kube-vip` in ARP mode is the usual choice for a plant network — no switch configuration required, just a reserved IP range on the same subnet as the nodes:
...
`destroy` removes the Flux kustomizations, then the VMs, in reverse order. Nothing in vCenter's own inventory (the datacenter, cluster, datastore, port group) is touched — Windsor only ever reads that.
...
- **VMs never get an IP in `windsor show`.** IPs come from `vmtoolsd` (VMware Tools) reporting back to vCenter, which needs the Talos OVA's bundled guest agent running — this can lag a boot or two. If it never resolves, confirm the ESXi host reached `factory.talos.dev` to pull the OVA in the first place.
- **OVF deploy fails outbound.** The ESXi host itself needs the network path to `factory.talos.dev`, not the machine running `windsor` — a common gap in segmented plant networks.
